...
class TestEstimationErrorMoC:
    ...
    def test_larger_sample_smaller_moc(self):
    ...
    def test_zero_observations_returns_pd(self):
    ...
    def test_high_confidence_larger_moc(self):
...
class TestDataRepresentativenessMoC:
    ...
    def test_zero_psi_only_dr_diff(self):
    ...
    def test_moderate_psi_adds_25bps(self):
        moc = data_representativeness_moc(
            development_default_rate=0.03,
    ...
    def test_high_psi_adds_50bps(self):
        moc = data_representativeness_moc(
            development_default_rate=0.03,
            application_default_rate=0.03,
            psi=0.30,
    ...
    def test_low_psi_no_penalty(self):
        moc = data_representativeness_moc(
            development_default_rate=0.03,
            application_default_rate=0.03,
            psi=0.05,
    ...
    def test_higher_psi_higher_moc(self):
...
class TestModelUncertaintyMoC:
    ...
    def test_zero_gap_when_challenger_lower(self):
        moc = model_uncertainty_moc(base_pd=0.05, challenger_pd=0.03)
        assert moc == pytest.approx(0.0, abs=1e-6)

    def test_proportional_to_gap(self):
    ...
    def test_multiple_challengers_shrinks_moc(self):
    ...
    def test_zero_challengers_treated_as_one(self):
...
class TestCalculateTotalMoC:
    ...
    def test_pd_capped_at_1(self):
        components = [
            MoCComponent(category=MoCCategory.ESTIMATION_ERROR, description="Huge", adjustment_bps=50_000),
        ]
...
class TestApplyMoCToPDCurve:
    ...
    def test_shifts_all_values_up(self):
    ...
    def test_floor_applied(self):
        ...
        moc_result = MoCResult(base_pd=0.0001, moc_additive=0.00005, adjusted_pd=0.0003, components=[])
        adjusted = apply_moc_to_pd_curve(curve, moc_result)
