...
class TestGetSupervisoryLGD:
    ...
    def test_senior_unsecured_no_collateral(self) -> None:
        exp = _make_exposure()
    ...
    def test_fully_secured_by_cash(self) -> None:
        exp = _make_exposure(
    ...
    def test_fully_secured_by_receivables(self) -> None:
        exp = _make_exposure(
    ...
    def test_fully_secured_by_rre(self) -> None:
        exp = _make_exposure(
            collaterals=[
    ...
    def test_fully_secured_by_other_physical(self) -> None:
        exp = _make_exposure(
            collaterals=[
    ...
    def test_partially_secured_50pct_cash(self) -> None:
        exp = _make_exposure(
    ...
    def test_zero_ead_returns_senior_unsecured(self) -> None:
        exp = _make_exposure(
            ead=0,
            collaterals=[Collateral(collateral_type=CollateralType.CASH, value=500_000)],
        )
    ...
    def test_zero_collateral_value_returns_senior_unsecured(self) -> None:
        exp = _make_exposure(
            collaterals=[Collateral(collateral_type=CollateralType.CASH, value=0)],
        )
        assert get_supervisory_lgd(exp) == 0.45

    def test_subordinated_ignores_collateral(self) -> None:
        exp = _make_exposure(
            collaterals=[Collateral(collateral_type=CollateralType.CASH, value=1_000_000)],
        )
        assert get_supervisory_lgd(exp, is_subordinated=True) == 0.75
...
class TestFoundationIRBCalculator:
    ...
    def test_basic_corporate_calculation(self) -> None:
    ...
    def test_defaulted_exposure_zero_rw(self) -> None:
        calc = FoundationIRBCalculator()
        exp = _make_exposure(
            is_defaulted=True, pd=1.0,
            drawn_amount=1_000_000, undrawn_commitment=0,
        )
        result = calc.calculate(exp)
    ...
    def test_ead_includes_ccf_times_undrawn(self) -> None:
        calc = FoundationIRBCalculator()
    ...
    def test_maturity_fixed_at_2_5_years(self) -> None:
        calc = FoundationIRBCalculator()
